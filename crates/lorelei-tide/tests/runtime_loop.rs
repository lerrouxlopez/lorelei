...
use lorelei_core::types::{
    AgentId, CurrentEvent, EchoHit, EchoQuery, PearlId, PearlType, Run, RunId, RunStatus,
    ShellCall, ShellResult, SongChunk, SongRequest, SongResponse, TenantId, UnitInterval,
};
...
struct CapturingSong {
    ...
    last_request: Mutex<Option<SongRequest>>,
}
...
async fn direct_answer_path() {
    ...
    let last = song.last_request.lock().unwrap().clone().unwrap();
    assert!(last.context.iter().any(|c| c.contains("remember this")));
...
async fn invalid_planner_json_repair_path() {
    ...
    let tide = SingleAgentTideRuntime::new(cfg, runs, currents, echo, song, shells, siren)
...
async fn siren_requires_approval_path() {
    ...
    let currents = Arc::new(MemCurrents::default());
    let echo = Arc::new(FixedEcho { hits: vec![] });
    let song: Arc<dyn SongProvider> = Arc::new(ScriptedSong::new(vec![r#"{"action":"call_shell","tool":"forget_pearl","input":{"pearl_id":"00000000-0000-0000-0000-000000000000"}}"#.to_string()]));
    let shells = Arc::new(FakeShells::default());
    let siren: Arc<dyn SirenPolicy> = Arc::new(DeterministicSirenPolicy::new(cfg.clone()));
